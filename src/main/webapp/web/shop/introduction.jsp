						...
						<div class="pay">
							...
							<li>
								<div class="clearfix tb-btn tb-btn-buy theme-login">
									<a id="LikBuy" title="点此按钮到下一步确认购买信息" href="javascript:order()">立即购买</a>
								</div>
							</li>
							<li>
								<div class="clearfix tb-btn tb-btn-basket theme-login">
									<a id="LikBasket" title="加入购物车" href="javascript:goods()"><i></i>加入购物车</a>
								</div>
							</li>
						</div>
...
var code = '<%=request.getParameter("code")%>';
 	$.ajax({
 		url:"/Text181130/controllerInformation/selectall.do",
 		data:{"commodity_code":code},
 		dataType:"json",
 		type:"post",
 		success:function(data){
 			$(".sys_item_price").html(parseFloat(data.commodity_price).toFixed(2));
 			$(".tb-detail-hd").html("<h1>"+data.commodity_name+"</h1>");
 			$(".stock").html(data.commodity_num);
 					$.ajax({
 						url:"/Text181130/commcodity_img/select.do?",
 						data:{"commodity_code":code},
 						dataType:"json",
 						type:"post",
 						success:function(img){
 							 var html="";
 							 var html1 = "";
 							$.each(img.img,function(i,dom){
 								if(dom.img_type==1){
 									html1+='<a href="/image/'+dom.img_url+'"><img src="/image/'+dom.img_url+'" alt="细节展示放大镜特效" rel="/image/'+dom.img_url+'" class="jqzoom" /></a>';
 									html+='<li class="tb-selected"><div class="tb-pic tb-s40"><a href="javascript:;"><img src="/image/'+dom.img_url+'" mid="/image/'+dom.img_url+'" big="/image/'+dom.img_url+'"></a></div></li>'
 								}else{
 									html+='<li><div class="tb-pic tb-s40"><a href="javascript:;"><img src="/image/'+dom.img_url+'" mid="/image/'+dom.img_url+'" big="/image/'+dom.img_url+'"></a></div></li>'
 								}
 							})
 							$("#thumblist").html(html);
 							$("#pic").html(html1);
 ...
 function goods(){
	 ...
	 if('${usercode}'==null||'${usercode}'==""){
		 alert("请登陆");
		 location.href="/Text181130/web/login.jsp"
	 }else{
// 		 console.log($("#text_box").val())
	 $.ajax({
		 url:"/Text181130/shoppingcart/insert.do",
		 data:{"commodity_code":code,"usercode":"${usercode}","num":$("#text_box").val()},
		 type:"post",
		 dataType:"text",
		 success:function(data){
			 console.log(data)
 ...
 $("#text_box").change(function(){
		if(!/^[1-9]\d*$/.test($("#text_box").val())){
			$("#text_box").val("1");
		}else if(parseInt($("#text_box").val())>parseInt($(".stock").html())){
			$("#text_box").val("1");
		}
 })
...
function ready() {
	...
	$("#add").click(function(){
		if($("#text_box").val()==parseInt($(".stock").html())-1){
	...
	function order(){
		 var code = '<%=request.getParameter("code")%>';
// 		 console.log(parseInt($(".sys_item_price").html())*parseInt($("#text_box").val()));
		location.href="/Text181130/web/shop/pay.jsp?code="+code+"&price="+parseInt($(".sys_item_price").html())*parseInt($("#text_box").val())+"&num="+$("#text_box").val();
	}
